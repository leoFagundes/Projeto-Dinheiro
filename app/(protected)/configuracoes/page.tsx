...
function AssinaturasSection() {
  ...
  return (
    <SectionCard icon={Repeat} title="Assinaturas" id="assinaturas">
      {assinaturas.length === 0 && paradas.length === 0 ? (
        <p className="text-sm text-ink-muted">
          Nada recorrente por aqui. Marque &ldquo;Repetir&rdquo; ao criar uma transação pra ela
        ...
        <>
          ...
          <ul className="flex flex-col gap-1.5 text-sm">
            {assinaturas.map((t) => {
              ...
              return (
                <li key={t.id} className="rounded-xl bg-bg px-3 py-2.5">
          ...
          {paradas.length > 0 && (
            <div className={assinaturas.length > 0 ? "mt-4" : ""}>
              <p className="mb-1.5 text-xs font-medium text-ink-muted">Pausadas</p>
              <ul className="flex flex-col gap-1.5 text-sm">
                {paradas.map((t) => (
                  <li
                    key={t.id}
                    className="flex items-center justify-between gap-2 rounded-xl bg-bg px-3 py-2.5 opacity-70"
                  >
                    <span className="min-w-0">
  ...
  return (
    <>
      ...
      <p className="mb-4 text-xs text-ink-muted">
        Isso para a assinatura atual e cria uma nova com os valores abaixo, a partir do mês
        escolhido. Nada do que já foi cobrado antes muda.
      </p>
